...
| `fullscreen` | `false` | `true` 使用 alternate screen、应用内滚动和鼠标选区；`false` 使用 inline 模式。也可在 `/settings` 面板切换（settings.yaml 的 `dsh-tui.fullscreen` 优先于此处；空闲时原地重载并保留会话，回合运行中则下次启动或重载时生效） |
...
| `DSH_TUI_DISABLE_MOUSE` | 在 fullscreen 模式临时关闭鼠标点击/拖选（滚轮滚动保留） |
