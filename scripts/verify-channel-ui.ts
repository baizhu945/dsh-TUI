...
// Real bare production startup, not raw createChannel passed to a renderer.
{
  ...
  const trajectory = mount.channel.trajectory()
  assert.equal(mount.channel.trajectory(), trajectory, 'trajectory seam keeps a stable build identity')
  assert.equal(mount.channel.rowsStreamingVersion, 0)
  assert.equal(mount.channel.rowsGeneration, 0)
