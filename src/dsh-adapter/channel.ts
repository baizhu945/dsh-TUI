...
export interface Channel {
  ...
  /**
   * The double-Esc session tree (pi's Session Tree): gather the live
   * session's family — its ancestor chain plus every descendant of the
   * topmost known ancestor — into one renderable tree. Read-only and
   * budgeted. `null` (with a notify) when session persistence is
   * unavailable, or silently when the live session swapped mid-build.
   */
  buildSessionTree(): Promise<SessionTreeData | null>
  /**
   * Rewind (double-Esc / /rewind / /tree) or fork (/fork) to an arbitrary
   * tree entry — possibly one of a dead branch (a real branch switch). A
   * `rewind` pick on a user message drops its whole turn and returns its
   * prompt text for re-editing; any other entry keeps through its enclosing
   * step. A `fork` pick KEEPS the entry itself: a user message cuts right
   * after it (the turn's reply drops), anything else behaves like rewind.
   * Returns the restored prompt text ('' when nothing is restored), or null
   * on refusal/failure (a notify carries the reason).
   */
  rewindToNode(sessionId: string, seq: number, mode?: 'rewind' | 'fork'): Promise<string | null>
  /**
   * Fork the CURRENT session at its tip into an independent, persisted copy
   * (kimi-code's `/fork`): the fork is created, titled `Fork: <source
   * title>`, and its runtime released immediately — the live session is
   * NEVER swapped (switching would kill its in-flight turn and background
   * work). The toast carries the fork's resume command; /resume lists the
   * copy right away. Refused (false, with a notify) while a turn runs —
   * forking needs a turn-closed log and cancelling the user's turn behind
   * their back would be a surprise.
   */
...
export interface ChannelState {
  ...
  rewindToNode(sessionId: string, seq: number, mode?: 'rewind' | 'fork'): Promise<string | null>
  /** @internal tip fork that stays in the source session (see the public Channel.forkSession). */
  forkSession(): Promise<boolean>
...
export function createChannel(
   ...
   * rebind subscriptions to the new agent, bump the replacement epoch, and
   * free the replaced handle. Returns the source session's id (for the
...
${output}
  ...
  const rewindToNodeDirect = state.rewindToNode
  ...
  // buildSessionTree is read-only and stays off the queue.
  state.rewindToNode = (sessionId, seq, mode) => enqueueSessionMutation(() => rewindToNodeDirect(sessionId, seq, mode))
