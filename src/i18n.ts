...
const dict = {
  ...
  'cmd-desc-rewind': { zh: '回退最后一轮（提示词回填重编）' },
  'cmd-desc-tree': { zh: '浏览会话家族树' },
  'cmd-desc-fork': { zh: '分叉当前会话为可恢复的副本' },
  ...
  'tree-unavailable': { zh: '会话树不可用——会话持久化服务未加载', en: 'Session tree unavailable — session persistence service not loaded' },
  'tree-fork-title': { zh: '从这里分叉？', en: 'Fork from here?' },
  'tree-fork-keep-message': { zh: '保留这条消息，丢弃该轮回复', en: 'Keeps this message; drops the rest of its turn' },
  'tree-forked': { zh: '已分叉：新会话保留了该条目', en: 'Forked: the new session keeps this entry' },
  'tree-hint': { zh: '**Enter** 回退 · ↑↓ 移动 · PgUp/PgDn 翻页 · {{mod}}o 过滤 · {{mod}}f 回退/分叉 · {{mod}}b 切到分支 · 输入搜索 · Esc 关闭', en: '**Enter** rewind · ↑↓ move · PgUp/PgDn page · {{mod}}o filter · {{mod}}f rewind/fork · {{mod}}b adopt branch · type to search · Esc close' },
  'tree-hint-confirm': { zh: '**Enter** 确认 · Esc 返回', en: '**Enter** confirm · Esc back' },
  'tree-mode-fork': { zh: '分叉模式', en: 'fork mode' },
} as const
